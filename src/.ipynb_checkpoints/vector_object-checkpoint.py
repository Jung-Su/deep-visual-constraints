...
class vectorObject(nn.Module):
    def __init__(self, **C):
        ...
        self.C = {}
        self.C['FEAT_IMG'] = 64
        self.C['WIDTH_LIFTER'] = [self.C['FEAT_CAM'], 512, 512, 128]
        self.C.update(C)
        self.build_modules()
    ...
    def forward(self, images, cam_params):
        ...
        cam_params = cam_params.view(-1, 8)
        ...
        return self.feature_lifter(feat_all) # (B, num_view, Feat)
...
class Pose_Feature_vec(nn.Module):
    ...
    def forward(self, poses, images=None, projection_matrices=None, grad_method=None):
        """
        Args:
            poses: (B, N, 7) poses
            images: (B, num_views, C, H, W) input images
    ...
    def optimize(self, 
        ...
        with torch.no_grad():
            ...
            for i in range(max_iter):
                ...
                w_coll = min(1.2*w_coll, 1e2)
                poses, delta_x = poses_tmp, delta_x_tmp,
                costs, colls = phi_tmp[:,:,0,0], phi_tmp[:,:,1,0]
                ...
                if i % print_interval == 0:
                    print('iter: {}, cost: {}, coll: {}'.format(i, costs.abs().max().item(), colls.max().item(), max_diff))
