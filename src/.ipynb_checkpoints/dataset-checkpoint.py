...
class PIFODataset(Dataset):
    def __init__(self, 
        ...
        self.cam_extrinsic = torch.from_numpy(data_hdf5['camera/cam_extrinsic'][:])
        self.cam_intrinsic = torch.from_numpy(data_hdf5['camera/cam_intrinsic'][:])
...
class RandomImageWarper:
    def __init__(self, img_res=None, sig_center=0.01, obj_r=0.15, return_cam_params=False):
        self.img_res = img_res
        self.sig_center = sig_center
        self.obj_r = obj_r
        self.return_cam_params = return_cam_params
...
def batched_random_warping(rgb, T1, K1, img_res=None, sig_center=0.01, obj_r=0.1, return_cam_params=False):
    """
...
#     K2[...,2,3] = -(cam_distance**2-obj_r**2)/obj_r
    ...
    # camera roll
    random_roll = torch.rand(B,num_views).to(device)*np.pi*2.
    rot_roll = axis_angle_to_matrix('Z', random_roll)
    T2[...,:3,:3] = T2[...,:3,:3].matmul(rot_roll)
    ...
    if return_cam_params:   
        cam_pos = T1[..., :3, 3]
        return rgb_warped.view(B, num_views, 3, H_out, W_out), K2.matmul(torch.inverse(T2)), cam_pos, new_origin, random_roll.view(B,num_views,1)
...
def batch_rotation_matrix(angle, axis):
    ...
    T[...,:3,:3] = axis_angle_to_matrix(axis, angle)
    return T
